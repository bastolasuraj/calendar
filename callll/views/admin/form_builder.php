<?php require_once 'views/templates/header.php'; ?>
                                ...
                                <?php foreach ($configurations as $config): ?>
                                    <tr>
                                        <td>
                                            ...
                                            <?php if ($config['active']): // Changed 'is_active' to 'active' ?>
                                            ...
                                            <div class="btn-group btn-group-sm" role="group">
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li><a class="dropdown-item" href="<?php echo BASE_PATH; ?>/admin/form_builder/export/<?php echo $config['_id']; ?>">
                    ...
                    <div class="row g-0 h-100">
                        ...
                        <div class="col-md-6">
                            <div class="p-3 h-100">
                                ...
                                <div class="form_builder-canvas bg-white border rounded p-4" id="formCanvas">
    ...
    <div class="modal fade" id="importModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="<?php echo BASE_PATH; ?>/admin/form_builder/import" method="POST" enctype="multipart/form-data">
    ...
    <script>
        let formBuilder = {
            ...
            selectedField: null,
            fieldCounter: 0,
            fieldTypes: <?php echo json_encode($fieldTypes); ?>, // Passed directly from PHP
            ...
            createNewForm() {
                this.currentForm = {
                    ...
                    settings: {
                        ...
                        active: false // Changed from 'is_active'
                    }
                };
        ...
        `;
            ...
            renderValidationRules(field) {
                const allowedRules = this.fieldTypes[field.type]?.properties?.filter(prop => ['min_length', 'max_length', 'min', 'max', 'pattern'].includes(prop)) || [];
                let html = '';

                if (allowedRules.includes('min_length')) {
                    html += `
                <div class="mb-2">
                    <label class="form-label form-label-sm">Minimum Length</label>
                    <input type="number" class="form-control form-control-sm" id="val_min_length" value="${field.validation.min_length || ''}" min="0">
                </div>
            `;
                }

                if (allowedRules.includes('max_length')) {
                    html += `
                <div class="mb-2">
                    <label class="form-label form-label-sm">Maximum Length</label>
                    <input type="number" class="form-control form-control-sm" id="val_max_length" value="${field.validation.max_length || ''}" min="0">
                </div>
            `;
                }
            ...
            updateFieldProperties() {
                ...
                field.validation = {};
                // Corrected way to get allowed validation rules based on `properties` array
                const allowedValidationRules = this.fieldTypes[field.type]?.properties?.filter(prop => ['min_length', 'max_length', 'min', 'max', 'pattern'].includes(prop)) || [];

                allowedValidationRules.forEach(rule => {
                    const input = document.getElementById(`val_${rule}`);
                    if (input && input.value) {
                        if (['min_length', 'max_length', 'min', 'max'].includes(rule)) {
        ...
        `;
            ...
            saveForm() {
                ...
                this.currentForm.settings.active = document.getElementById('isActiveForm').checked; // Changed from 'is_active'
                ...
                const url = this.currentForm.id ?
                    `${BASE_PATH_JS}/admin/form_builder/update/${this.currentForm.id}` :
            ...
            previewForm() {
                ...
                fetch(`${BASE_PATH_JS}/admin/form_builder/preview`, {
                    method: 'POST',
                    headers: {
        ...
        function editForm(configId) {
            ...
            fetch(`${BASE_PATH_JS}/admin/form_builder/getConfiguration/${configId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('isActiveForm').checked = data.configuration.active; // Changed from 'is_active'
        ...
        function previewForm(configId) {
            ...
            fetch(`${BASE_PATH_JS}/admin/form_builder/getConfiguration/${configId}`)
                ...
                .then(data => {
                    if (data.success) {
                        return fetch(`${BASE_PATH_JS}/admin/form_builder/preview`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
        ...
        function duplicateForm(configId) {
            if (confirm('Do you want to create a copy of this form?')) {
                window.location.href = `${BASE_PATH_JS}/admin/form_builder/duplicate/${configId}`;
            }
        }

        function activateForm(configId) {
            if (confirm('Do you want to set this form as the active booking form?')) {
                window.location.href = `${BASE_PATH_JS}/admin/form_builder/setActive/${configId}`;
            }
        }
        ...
        function deleteForm(configId) {
            if (confirm('Are you sure you want to delete this form configuration? This action cannot be undone.')) {
                window.location.href = `${BASE_PATH_JS}/admin/form_builder/delete/${configId}`;
            }
        }
